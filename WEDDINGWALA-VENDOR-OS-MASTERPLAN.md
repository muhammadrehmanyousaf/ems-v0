...
| 41 | 2026-05-25 | "keep going" | Built **WhatsApp Template-Performance Analytics** — the closest we can get to "did this template work?" while staying on free wa.me (decision D3, no Business API). Audit-first confirmed clean slate: templates are hardcoded in `lib/whatsapp.ts`, no existing send-log, leadController's `sendLeadWhatsapp` is dead code for template tracking. New migration `20260525120000-whatsapp-send-logs.js` (applied to Neon): `WhatsappSendLogs` table with vendorUserId, businessId, templateKey+Label, targetType (lead/customer/booking/sheet/free), targetId, phoneTo, soft pointers (leadId/bookingId/customerId/functionSheetId), messagePreview (500 chars), sentAt + 4 indexes. New `POST /api/v1/whatsapp/send-log` (fire-and-forget, vendor-scoped, 500-char preview). New `GET /api/v1/analytics/whatsapp-templates?range=last_90_days` aggregates: sends/template + leads-linked + leads-booked + bookings-linked + bookings-Completed/Paid → rates (returns null when no denominator to avoid 1/1=100% trap) + byTargetType + recentSends(8). BE `460859e`. FE `ee67a03`: `WhatsappAPI.logSend` (swallows errors so wa.me never fails) + `WhatsappAnalyticsAPI.getTemplatePerformance` types; `whatsapp-quick-send.tsx` gained optional lineage props (targetType, targetId, leadId, bookingId, customerId, functionSheetId) — default targetType="free" preserves back-compat; booking detail view passes `targetType="booking"+bookingId`; customer detail view passes `targetType="customer"+customerId`; `<WhatsAppTemplatePerformance/>` (~200 lines) on /dashboard/insights: hero (total sends · most-used emerald · "where you message" target-type chips), per-template table (6 cols: label·key · sends · lead→booked rate w/ tone scale · booking→paid rate · last send relative time) with footnote about denominator, recent-sends-8 timeline w/ colored target badges. Uses existing `NEXT_PUBLIC_WA_TEMPLATES` flag (directive #21) — no new flag |
