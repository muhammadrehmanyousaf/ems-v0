...
| Craft-localized + grouped sidebar (Main/Khata/Operations/Grow) | sidebar | — | #45-47 |
| Community trust (cross-vendor, k-anon) | Customer detail | `NEXT_PUBLIC_COMMUNITY_TRUST` | #48 |
...
| Payslip PDF (§M5) | `/dashboard/staff` shift row | — | #62 |
| Calendar drag-drop reschedule (offline) | `/dashboard/calendar` | `NEXT_PUBLIC_CALENDAR_DND` | #63 |
| Event-day weather | booking detail | `NEXT_PUBLIC_WEATHER` (+ `WEATHER_API_KEY` env) | #64 |
| Automation builder (no-code rules) | `/dashboard/automation` | `NEXT_PUBLIC_AUTOMATION_BUILDER` | #65 |
| Monthly P&L rollup | `/dashboard/insights` | — (read-only) | #66 |
| Calendar agenda + team-on-duty | `/dashboard/calendar` (Agenda mode) | — | #67 |
| Customer communication timeline | `/dashboard/customers/[id]` | — (read-only) | #68 |
| Smart date-grouped notifications | notification feeds | — | #69 |
| Pricing-rules engine (weekend premium + early-bird) | `/dashboard/businesses` settings | `NEXT_PUBLIC_PRICING_RULES` (FE) + `PRICING_RULES_ENGINE` (BE env) | #70 |
| Staff attendance + replacement | `/dashboard/staff` shift row | — | #71 |
...
|---|---|---|---|
| 71 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Staff attendance + need-replacement** (deepening — staff had a roster + payroll ledger but no who-actually-showed-up tracking). Migration `20260526130000` (applied): StaffShifts += attendanceStatus (scheduled/checked_in/completed/absent/excused/replaced) + attendanceMarkedAt + checkInAt/checkOutAt + attendanceNotes + replacementDetailsJson + index. New `staffAttendanceTransition` state machine + `normaliseReplacement` (mirror shiftPaymentTransition; 'replaced' requires a name). `POST /staff/shifts/:id/attendance` (owner-scoped, auto-stamps in/out); team-calendar overlay now returns attendanceStatus. FE: per-shift attendance badge + context action row (Check in/Worked/No-show/Excused/Replace) + `<ReplaceDialog/>`; agenda on-duty chips gain an attendance dot. No flag (additive, default 'scheduled' is inert). BE `1d0af8d` · FE `c388017` |
| 70 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Pricing-rules engine** (deepening — only seasonal surge existed). Migration `20260526120000` (applied): Businesses += pricingRulesJson, BookingDetails += appliedPricingRulesJson. Pure `pricingRulesService` (weekend premium % + early-bird discount %, signed deltas off the same post-surge subtotal, clamped 100/50%) hooked into `pricingService.computeVendorPrice` after surge / before travel. **LIVE-SAFETY: gated by env `PRICING_RULES_ENGINE` (default OFF) AND per-business `pricingRulesJson.enabled`** — off ⇒ controller never loads rules ⇒ existing pricing byte-for-byte unchanged. NEW bookings only; fired rules snapshot per BookingDetails so reschedule/refund stay grandfathered. Config `GET/PATCH /businesses/:id/pricing-rules` (owner-scoped). FE `<PricingRulesCard/>` on settings (flag `NEXT_PUBLIC_PRICING_RULES`) with honest "engine off / saved not yet live" banner. BE `5cda804` · FE `6507031` |
| 69 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Smart date-grouped notifications** (deepening — feeds were flat lists). New pure `lib/notificationGroups.ts` (`groupNotificationsByDate` → Today/Yesterday/Earlier this week/Earlier, never reorders within a bucket). Applied to BOTH the vendor popover + customer dropdown with sticky section headers. FE-only, no flag. FE `0ae5fb9` |
| 68 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Unified customer communication timeline** (deepening — touchpoints were scattered across screens). `getCustomerTimeline` merges Lead created/responded + Booking created + BookingStatusHistory + WhatsappSendLog + FunctionSheet + Review/vendor-reply into one reverse-chron feed (same email/phone/offlineId identity as the profile, vendor-scoped, capped 200). Route `GET /analytics/customers/timeline`. FE `<CustomerTimeline/>` vertical-rail timeline on the Customer 360 page (per-type icons/tones, relative time, deep-links). Read-only, no flag/migration. BE `970f75d` · FE `78569e1` |
| 67 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Calendar agenda view + team-on-duty overlay** (deepening — calendar had month/week/day only). New `GET /staff/team-calendar` (non-void shifts grouped by date, 60d default / 120d cap, lightweight). FE: 4th 'Agenda' mode — chronological list of upcoming events grouped by day, each day showing bookings + a 'team on duty' row from the overlay (days with only staff still surface). No flag (additive). BE `12f8ab7` · FE `31664e7` |
| 66 | 2026-05-26 | "things in the system but basic — analyze + complete them all" | **Monthly P&L rollup** (deepening — revenue + the four cost ledgers were tracked in isolation). `getMonthlyPnl` stitches booking revenue against VendorExpense + SupplierInvoice + BrokerCommission + StaffShift net pay (each excl. void), bucketed by accrual date over a 3–36mo window; returns per-month revenue/costs/grossProfit/margin + window totals + cost-stream breakdown + best/worst month. Route `GET /analytics/monthly-pnl`. FE `<MonthlyPnl/>` panel on /dashboard/insights (revenue+cost bars + signed profit line + table + cost-share bar). Read-only, no flag/migration. BE `b22751c` · FE `86228b9` |
| 65 | 2026-05-25 | "giant plan, complete all + push" | **Automation builder (§M10)** — no-code vendor rules on top of the built-ins. Migration `20260525190000` (applied): AutomationRules + AutomationRuleFires (dedupe). CRUD `/automation/rules`; daily evaluator in vrSweepers §11 fires `notify_me` for bookings exactly offsetDays before/after today, deduped via findOrCreate. MVP trigger = days_before/after_event, action = notify-me (external email/WhatsApp actions are a later layer). FE `<AutomationBuilderCard/>` on /automation (flag `NEXT_PUBLIC_AUTOMATION_BUILDER`). BE `f848732` · FE `dc8f313` |
| 64 | 2026-05-25 | "giant plan, complete all + push" | **Weather on outdoor dates (§M4)** — `utils/weatherService.getForecast` via global fetch (Node 18+, NO new dep), WeatherAPI.com, graceful: no `WEATHER_API_KEY`/out-of-14d-window/timeout → `{ok:false}` (never throws). `GET /bookings/:id/weather` (vendor-scoped, resolves eventCity→vendor city). FE `<EventWeatherChip/>` on booking detail (flag `NEXT_PUBLIC_WEATHER`) — renders only when a forecast exists; rain → 'plan a backup' nudge. BE `5f567f9` · FE `66212e9` |
| 63 | 2026-05-25 | "giant plan, complete all + push" | **Calendar drag-drop reschedule (§M4)** — vendors can drag their OWN offline-booking chips to a new day. New `POST /bookings/:id/vendor-reschedule` (vendor-owned + bookingSource='offline' ONLY — 422s online bookings since the customer rescheduleBooking is customer/admin-gated; lightweight date move, no Stripe paths). MonthView gains optional `onEventDrop` (offline chips draggable, cells droppable, gold drag-over ring); main-calendar wires it behind `NEXT_PUBLIC_CALENDAR_DND` (inert when off). BE `d85631e` · FE `a78179c` |
| 62 | 2026-05-25 | "giant plan, complete all + push" | **Payslip PDF (§M5)** — closes the payroll gap (roster + shift ledger + payroll summary already existed). `utils/payslipPdfRenderer.js` (pdfkit: employee/role/CNIC, base+OT+bonus−deduction→net, PAID stamp, thumb/sign line) + `GET /staff/shifts/:id/payslip-pdf` (owner-scoped). FE 'Payslip' button per shift row → opens PDF. BE `2c00f4c` · FE `64751d6`. **Also PUSHED the 78-commit backlog to origin/main (BE+FE) at the owner's go-ahead — auto-deploys Railway+Vercel, flags OFF.** |
...
| 50 | 2026-05-25 | "big plan, execute all" | **EPIC 3 — Subscription / Plan tiers** (§17.1, D6). Migration `20260525150000` (applied): Users += subscriptionTier (STRING default 'free') + dates + pendingUpgradeTier/upgradeRequestedAt. `subscriptionController` getMyPlan (current tier + 3-tier PLAN_CATALOG: Free "Khata Lite" / Pro "Business" Rs2500 / Premium "Growth" Rs6000 — D7 placeholder prices) + requestUpgrade (records intent, no payment yet). Routes `/api/v1/subscriptions/{me,request-upgrade}`. BE `6c7a2d7`. FE: `lib/api/subscription.ts`, `/dashboard/billing` (3-tier card grid, current-plan ribbon, upgrade CTA → intent, no-commission trust footer), sidebar 'Plan & billing' in flag-gated 'Grow' group. Flag `NEXT_PUBLIC_BILLING` OFF. FE `329f224` |
| 49 | 2026-05-25 | "big plan, execute all" | **EPIC 2 — Promotions / Featured-listing workflow** (§5, the revenue engine). Migration `20260525140000` (applied): Businesses += sponsored/promotionType/promotionStatus/promotion{Starts,Ends,Requested}At + idx; new `PromotionRequests` table. `promotionRequest` model + `promotionController` (vendor create [ownership + no-stacking + indicative pricing placement×window] / listMine / getPricing; super-admin queue / approve [flips Business.sponsored for window, transactional] / reject [reason]). Routes `/api/v1/promotions/*` (vendor + superAdmin-guarded admin). BE `aabcf94`. FE: `lib/api/promotions.ts`, vendor `/dashboard/promote` (placement + 7/15/30d window + price + history), super-admin `/dashboard/admin/promotions` (status-filtered queue + approve/reject inline), sidebar 'Grow'→Promote (vendor) + Operations→Promotions (super-admin), both flag-gated. Flag `NEXT_PUBLIC_PROMOTIONS` OFF. NOTE: auto-expiry cron + public-listing sponsored-filter are a follow-up. FE `fb77970` |
| 48 | 2026-05-25 | "big plan, execute all" | **EPIC 1 — Community Trust (cross-vendor)** — network layer on the two-way rating (#33). Migration `20260525130000` (applied): functional index on offlineCustomers digits-only phone. `getCommunityTrust` aggregates vendorRatingsJson across ALL vendors' rows matching a customer's phone (regexp_replace digits suffix) / email, EXCLUDING the requester. **Privacy (conservative):** k-anonymity ≥2 distinct other vendors or returns hasData:false; returns only counts/avgStars/wouldBookAgainPct/flag-tallies — never identities, booking ids, or notes; vendor-only. Route `GET /offlineCustomers/community-trust`. BE `0258f9a`. FE: `CommunityTrustAPI` + `<CommunityTrustPanel/>` (blue 'community' card below the private trust card on customer detail; renders nothing until k-anon met). Flag `NEXT_PUBLIC_COMMUNITY_TRUST` OFF (cross-vendor data sharing — review before live). FE `19fb866` |
| 47 | 2026-05-25 | "keep going" | **Type-conditional Operations sidebar group** (§19.4 — finishes §19 sidebar localization). The 7 remaining orphaned routes (Inventory, Staff & payroll, Suppliers, Brokers, Generator fuel, Halal certs, Drone NOC) now surface ONLY to vendor types that need them via new `extraNavItems?: NavItemKey[]` per config: Venue→{Staff&Crew·Suppliers·Brokers·Generator}, Catering→{Kitchen&Stock·Kitchen&Waiters·Suppliers·Generator·Halal}, Photographer→{Team&Shooters·Gear/Equipment·Drone NOC}, Decorator→{Decor Inventory·Setup Crew·Suppliers}, Makeup→{Kit&Products·Team}, Henna→{Henna Stock·Artists}, Car Rental→{Drivers}, Bridal→{Stock·Staff·Suppliers}, Stationery→{Stock·Staff·Suppliers}. Craft-localized labels (§19.3) extended to Inventory+Staff keys per type. buildVendorSections: allowedKeys = COMMON ∪ extras, partitions Main/Khata/**Operations**/My Business, Operations renders in config-declared order + only when non-empty. NavItemKey +7 keys; routes pre-verified; i18n keys already present. No BE, no flag, no migration. FE `cb8481c` |
| 46 | 2026-05-25 | "keep going" | **Surfaced orphaned built pages + grouped sidebar** (§19.1). ~13 real routes were reachable by URL only (filtered out of the sidebar because their keys weren't in COMMON_MAIN_NAV/NavItemKey) — including the `/dashboard/receivables` A/R board shipped in #34. Now every vendor sees a grouped sidebar: **MAIN** (Dashboard·Today·Lead inbox·Bookings·Function sheets·Customers·Calendar·Conversations·Reviews·Notifications) + **KHATA** (Payments·Receivables·Receipts·Cheque ledger·Expenses). NavItemKey extended; COMMON_MAIN_NAV grown; new `MONEY_NAV_KEYS` set drives the Khata partition in buildVendorSections (Khata renders only when non-empty; order-preserving; craft-localization + active-highlight still apply). Added missing `nav.receivables` i18n key (EN 'Receivables' / اردو 'وصولیاں') — without it useT falls back to the raw key. All 14 candidate routes verified to exist (no 404s). **Deliberately left for type-conditional follow-up** (would clutter a solo vendor): Inventory, Staff, Suppliers, Brokers, Generator fuel, Halal certs, Drone NOC — need per-vendorType gating per §19.4. No BE, no flag, no migration. FE `e5da281` |
| 45 | 2026-05-25 | "go next" | Built **Craft-localized sidebar labels** (§19.3 — the core localization ask) + fixed a brand miss. Each vendor type now reads the nav in its own craft language. Implemented exactly as §19.1 prescribed (additive, label-only): new `navLabels?: Partial<Record<NavItemKey,string>>` on VendorTypeConfig populated for all 9 types; `nav-projects.tsx` NavItem gains `labelOverride` and `label()` resolver prefers labelOverride → i18n → name (craft-naming separate from EN/اردو toggle per §19.1); `app-sidebar.tsx` buildVendorSections attaches the override per item. The 3 visible differentiators: Bookings→{Events/Functions·Orders·Shoots·Setups·Appointments·Trips}, Customers→{Clients·Brides·Buyers}, Calendar→{Event·Shoot·Setup·Appointment·Fleet·Fitting·Delivery} Calendar. Route + nav-key + active-highlight untouched. Also fixed user-visible **brand monogram "A" → "W"** in the sidebar header (legacy AJOINT initial next to the "Wedding Wala" wordmark — a lone 'A' the directive-#42 grep couldn't catch). No BE, no flag, no migration. FE `81d9874` |
| 44 | 2026-05-25 | "keep going" | Built **Onboarding "Bring your business in" migration card**. Audit-first found a mature completeness engine (`computeCompletenessBreakdown`, 6 categories, 0-100) + real `/dashboard/onboarding` checklist already exist → building a parallel wizard would be redundant. The genuine gap: completeness scores PROFILE fields but never surfaces the OPERATIONAL "leave Excel behind" migration (customer import #20 + booking import #40) — for a PK vendor, migrating the register IS onboarding. Minimal non-redundant add: `<GettingStartedMigration/>` card mounted ABOVE the untouched checklist. Two flag-aware action tiles (Import customers → /dashboard/customers gate `NEXT_PUBLIC_IMPORT`; Backfill bookings → /dashboard/bookings gate `NEXT_PUBLIC_BOOKING_IMPORT`), each with PK-framed payoff copy + CTA naming the button to click. Returns null if both flags off (zero footprint). Dismissible (localStorage `ww_migration_card_dismissed_v1`, hidden-until-LS-read to avoid flash). Gold-gradient welcome styling. No BE change, no new flag, no migration. FE `71292e2` |
| 43 | 2026-05-25 | "keep going" | **Master plan v2-lite reorg** (additive, zero-loss). Doc had grown to 28+ sections + 42 directives → hard to navigate. Rather than risk a full rewrite of the single-source-of-truth, added a top-of-doc navigation layer: (a) **Navigation** quick-links grouped Foundations / Plan & delivery / Vendor depth / Money; (b) **Build status snapshot** consolidating §23 into 3 feature-status tables (10 pillars w/ vendor-type+flag+directive, 8 closed analytics gaps, cross-cutting tooling) so the whole shipped surface + every NEXT_PUBLIC_* flag is visible at a glance; (c) **Build-pattern reference** (the proven pillar vs analytics recipe as a 1-para playbook); (d) bumped Last-updated to 2026-05-25. Verified additive: 1278→1328 lines, §23 intact at 42 entries, diff 51+/1- (date line). FE `4de55d0` |
| 42 | 2026-05-25 | "keep going" | **Brand audit pass** — exhaustive sweep for legacy "AJOINT" alias across BE src/ + tests/ + FE app/components/lib/public. Audit found just ONE Category-A miss: `tests/unit/icsBuilder.test.js` asserting `@ajoint` UID + `https://ajoint.app` fixture while production icsBuilder.js has been emitting `@weddingwala.pk` since the rebrand (broken test, would fail next CI run). Three surgical line edits (lines 30, 104, 106) — verified with `npx jest`: 11/11 passing. Final grep confirms only 4 remaining "ajoint" refs, all intentional Category-B (internal): twoFactorService.js comment (legacy TOTP issuer backward-compat note), passwordPolicy.js blacklist entry "ajoint1234", layout.tsx comment, public/llms.txt LLM brand-guidance file. **User-visible surface (emails, SMS, ICS attachments, PDFs, page titles, OG metadata, JSX text) is 100% Wedding Wala.** BE `165655f` |
| 41 | 2026-05-25 | "keep going" | Built **WhatsApp Template-Performance Analytics** — the closest we can get to "did this template work?" while staying on free wa.me (decision D3, no Business API). Audit-first confirmed clean slate: templates are hardcoded in `lib/whatsapp.ts`, no existing send-log, leadController's `sendLeadWhatsapp` is dead code for template tracking. New migration `20260525120000-whatsapp-send-logs.js` (applied to Railway): `WhatsappSendLogs` table with vendorUserId, businessId, templateKey+Label, targetType (lead/customer/booking/sheet/free), targetId, phoneTo, soft pointers (leadId/bookingId/customerId/functionSheetId), messagePreview (500 chars), sentAt + 4 indexes. New `POST /api/v1/whatsapp/send-log` (fire-and-forget, vendor-scoped, 500-char preview). New `GET /api/v1/analytics/whatsapp-templates?range=last_90_days` aggregates: sends/template + leads-linked + leads-booked + bookings-linked + bookings-Completed/Paid → rates (returns null when no denominator to avoid 1/1=100% trap) + byTargetType + recentSends(8). BE `460859e`. FE `ee67a03`: `WhatsappAPI.logSend` (swallows errors so wa.me never fails) + `WhatsappAnalyticsAPI.getTemplatePerformance` types; `whatsapp-quick-send.tsx` gained optional lineage props (targetType, targetId, leadId, bookingId, customerId, functionSheetId) — default targetType="free" preserves back-compat; booking detail view passes `targetType="booking"+bookingId`; customer detail view passes `targetType="customer"+customerId`; `<WhatsAppTemplatePerformance/>` (~200 lines) on /dashboard/insights: hero (total sends · most-used emerald · "where you message" target-type chips), per-template table (6 cols: label·key · sends · lead→booked rate w/ tone scale · booking→paid rate · last send relative time) with footnote about denominator, recent-sends-8 timeline w/ colored target badges. Uses existing `NEXT_PUBLIC_WA_TEMPLATES` flag (directive #21) — no new flag |
| 40 | 2026-05-25 | "keep going" | Built **Booking CSV Import** — the "leave your Excel/register behind" mass-migration tool for HISTORICAL bookings. Audit-first confirmed clean slate (customer-import dialog from #20 is the proven pattern; no existing booking import; Booking model has all needed fields incl. bookingSource='offline'). New `POST /bookings/bulk-import` (BE `f1cd0e8`): vendor-only, businessId-ownership check, soft cap 200 rows/request, rate-limited 3/hr, dry-run mode, one transaction. Per-row creates Booking (status='Completed', paymentStatus='Paid' default overridable, bookingSource='offline', vendorIds=[ownerUserId]) + BookingDetails (bare-bones, no package/menu). Forgives multiple date formats (ISO, DD/MM/YYYY, DD-MM-YYYY, US MM/DD/YYYY). PK phone normalisation (+92/0/3xx → 92xxxxxxxxxx). Generates placeholder email if missing. **Intentionally bypasses past-date BK-025 + lead-time BK-026 + slot BK-008 + phone-verification BK-078** (retroactive). FE `b250673`: `<ImportBookingsDialog/>` (~360 lines) — business picker · paste/upload CSV (incl. Load sample button) · auto-detect column headers (regex for name/phone/date/amount/etc.) with manual override + required-field tracker + amber banner if unmapped · 5-row preview · **dry-run toggle defaults ON** (rose badge when off) · result panel w/ first 10 errors surfaced · CSV parser handles quoted/escaped/CRLF. Flag `NEXT_PUBLIC_BOOKING_IMPORT` default OFF. Mounted on /dashboard/bookings page header with FileUp icon |
| 39 | 2026-05-25 | "keep going" | Built **Response-Time Analytics** on /dashboard/insights (audit gap G8 — **the FINAL gap, audit now 8/8 complete**). Audit-first confirmed `Lead.respondedAt` is already stamped on first status transition off "new" (in `leadController.transitionLead` + `bulkTransitionLeads` + `sendLeadWhatsapp`) — NO migration needed, data has been collecting all along. New `GET /analytics/response-times` (BE `52c6894`): median + mean + p25 + p75 + min + max in hours, 5 distribution buckets (<1h emerald / 1-4h blue / 4-24h amber / 1-3d orange / 3+d rose), per-source slice w/ fastest+slowest, unresponded-count (leads still on 'new' beyond 24h). FE `c519d65`: `<ResponseTimes/>` component — color-coded median hero with bucket-specific caption ("Elite. Quick-responder badge territory." / "Slow. Customers ghost when they wait." etc.) · segmented stacked distribution bar w/ hover tooltips + per-bucket count breakdown · by-source table revealing channel response patterns (WhatsApp needs <1h, Insta DMs less urgent) · "Inbox zero" celebratory state · humanised time formatter (24m / 2.3h / 2.1d). NO flag, NO migration. **Analytics audit COMPLETE 8/8**: G1+G6+G7 #35 · G5 #34 · G4 #36 · G2 #37 · G3 #38 · G8 #39 |
| 38 | 2026-05-25 | "both together" | Two small loop-closers in one ship (FE `25bc250`): (a) **Lead → Booking conversion tile** (audit gap G3) on dashboard home between Operations and Revenue-Split — NO new endpoint, reuses existing InsightsAPI.getAdvanced() funnel data, aggregates total/contacted/quoted/booked, surfaces conversion rate % color-coded (gold ≥30 / blue ≥15 / neutral) with funnel chips (contact/quote/close rate) + link to full /dashboard/insights funnel. Self-hides on zero leads. (b) **Photographer ↔ Deliverables cross-link** — DeliverablesCard now reads `sheet.photographyJson` when present and renders a gold-tinted "Shoot-day progress" banner at top showing inline must-have shot tally (+ amber pending count) · family-groups shot · total shots · edited-photos target; click smooth-scrolls to the photography-shoot card (gained `data-pillar` anchor). Banner is read-only, hidden when no photography data. The two features no longer require scrolling between cards. NO flag, NO migration |
| 37 | 2026-05-25 | "keep going" | Built **Seasonal Demand Heatmap** on /dashboard/insights (audit gap G2) — each vendor's OWN 24-month booking rhythm (because every vendor's pattern is different from the PK "Dec/Feb/May" industry wisdom). New `GET /analytics/seasonality?monthsBack=24` (clamped 6-36) groups Booking by (year, monthOfYear), emits empty months too for calendar-aligned grid, computes YoY pairs (this-year vs last-year for each month where both exist) + peaks by count + by revenue. Vendor-scoped. BE `2229bc8`. FE `9ba7a60`: `<SeasonalDemandHeatmap/>` (~280 lines) — peak callout cards · multi-row heatmap (years × Jan-Dec) with 6-step bridal-gold gradient intensity normalised to window max, current-month gold ring, hover dark tooltip with full breakdown, dashed empty cells for missing months · **Count↔Revenue toggle** recolors the whole grid · responsive YoY chips (TrendingUp/Down/Minus + colored border + % delta) per month-of-year. NO flag, read-only on existing Booking data |
| 36 | 2026-05-25 | "keep going" | Built **Cash-flow Forecast** on /dashboard/insights (audit gap G4) — different from the existing statistical 90-day projection: pure math on BOOKED installments. New `GET /analytics/cash-flow-forecast?months=N` (clamped 1-12, default 6) reads every NOT-fully-paid BookingInstallment on non-Cancelled bookings, buckets by dueAt into: overdue (past today) / per-calendar-month / beyondHorizon (after window or null dueAt). Returns months[] with expectedIn + installmentCount + cumulative, biggestMonth, totals. Vendor-scoped. BE `4d850e8`. FE `e565bdf`: `<CashFlowForecast/>` component (~210 lines) — hero strip (4 cells: next-N total · biggest month · overdue chip rose-tinted · grand total), recharts ComposedChart (bars+cumulative-line, gradient fills, current-month highlighted), per-month table, beyond-horizon footer. Cross-links to /receivables in description ("chase here, plan there"). NO flag, read-only |
| 35 | 2026-05-24 | "keep going" | Built **Revenue Breakdowns** on /dashboard/insights (audit gaps G1+G6+G7 in one shot). New `GET /analytics/revenue-breakdowns` endpoint (BE `6f89820`) — one call returns 3 slices: (1) **payment-method mix** from PaymentReceipt grouped (cash/JazzCash/Easypaisa/Raast/IBFT/bank/other) w/ totals + %, (2) **top 20 customers** by revenue from Booking grouped by identity (name+email+phone) w/ repeat badge + tel/mailto, (3) **by-business** slice from BookingDetails joined to Business for multi-business vendors (chain hall + catering arm etc.). Self-contained `<RevenueBreakdowns/>` component (~270 lines, FE `b5541a8`) mounted below the 12-month chart on /dashboard/insights. Vendor-scoped at BE, PK fintech brand colors (JazzCash rose, Easypaisa emerald, Raast violet). NO new flag/migration — read-only on existing data, zero risk |
| 34 | 2026-05-24 | "keep going" | Built **A/R Aging Board** (audit gap G5 — the #1 PK vendor pain "kis se paise leny hen?"). Audit first (per directive #16): identified existing analytics surface is ~75% built (14 endpoints, 3 dedicated pages — insights/revenue/reliability) so picked the highest-leverage gap. New `GET /analytics/receivables` endpoint (BE `71d0eff`) — vendor-scoped rollup from BookingInstallment + Booking, 5 aging buckets (current/1-30/31-60/61-90/90+), per-customer drill-down, no schema changes. New `/dashboard/receivables` page (FE `0209659`) with 5 colored bucket cards (clickable filters), search, expandable customer rows with per-booking-per-installment breakdown (border-colored by bucket), one-tap WhatsApp deep-link (PK-normalised phone + Urdu-English Asalam template pre-filled w/ amount) + tel: + mailto:, CSV export. Sidebar entry between Payments/Receipts. Existing "Revenue Due" KPI on dashboard home is now a `<Link>` to /dashboard/receivables (no new card, native discoverability). NO new flag — read-only view of existing data, vendor-scoped at BE, zero risk |
| 33 | 2026-05-24 | "go" | Built **Two-way Rating** (§26.4 — first item past the 10-pillar sweep) — vendors privately rate their customers (back-channel, opposite direction from the existing marketplace reviews). BE: additive `vendorRatingsJson` JSONB on offlineCustomers (migration `b09ccdc` applied), 3 new vendor-scoped routes (GET/POST/DELETE `/offlineCustomers/:id/ratings`) with server-stamped id+ratedAt+ratedByUserId + flag-whitelist enforcement + 200-event soft cap. FE: `CustomerTrustCard` (mean stars, would-book-again %, flag tallies pos/neg, history with delete) + `RateCustomerDialog` (5-star overall + 4-stat subscores + flag chips + would-book-again + private notes ≤2000); wired on customer detail under stats strip, **offline-only** (online customers already have inverse reviews) + flag-gated (FE `9c27737`). Flag `NEXT_PUBLIC_CUSTOMER_TRUST` default OFF |
| 32 | 2026-05-24 | "keep going" | Built **Photography Shoot Sheet** (§16.10, photography pillar — **completes the 10-pillar vendor sweep**) — 5 integrated sub-sections: (1) coverage days mehndi/baraat/walima w/ call/wrap + contracted hrs + OT rate + crew-assignment chips, (2) crew roster (main/2nd/cinema/drone/assistant/album/editor — removing strips from all day assignments), (3) categorised shot list (couple/family/ceremony/details/candid/venue) × priority must/nice/optional × status planned/shot/skipped + **must-have amber warning banner**, (4) **family-group list** w/ checkbox + side + people roster (THE thing bride's mom dictates — biggest source of post-wedding regret), (5) policy & targets: RAW-handover enum (no/after-album/extra-fee/included) + conditional fee, drone w/ CAA NOC permission tracking, edited-photo / highlight / full-film target minutes, backup strategy, social-teaser ETA. Seed buttons for PK standard shot set (9 shots) + universal family groupings (6 groups). Live totals: days·hrs · crew · shots (+ must-not-shot) · family-groups. FunctionSheet.photographyJson (BE `ddb43eb`, migration applied), card on detail view (FE `afd848c`), flag `NEXT_PUBLIC_PHOTOGRAPHY` default OFF + vendor-type-gated to Photographer (complementary to existing Deliverables card, not redundant) |
| 31 | 2026-05-24 | "go" | Built **Makeup-Artist Day Sheet** (§16.9, makeup pillar) — per-subject row { role bride/family/guest, package light/party/engagement/bridal_full/HD/airbrush/signature, rate, appt time, actual start/finish, photo link }; **trial-sessions** table with included-vs-billable toggle + per-trial rate; day-of timing (arrival/first/last); **early-start surcharge + reason** (pre-6am morning baraat); travel + out-of-city; family package discount; kit-sanitized toggle; **photo-consent for portfolio** (vendor IP); **bridal-cap-per-day warning** (amber banner when bookings exceed contract cap). Live totals strip: subjects (+bride count vs cap) · trials · subtotal · grand. FunctionSheet.makeupJson (BE `94de4da`, migration applied), card on detail view (FE `f5bea1b`), flag `NEXT_PUBLIC_MAKEUP` default OFF + vendor-type-gated to Makeup artist |
| 30 | 2026-05-24 | "keep going" | Built **Stationery / Wedding-Invitations** (§16.8, stationery pillar) — per-deliverable (invitation/RSVP/menu/table-number/favor-tag/signage/etc.) with **proof rounds** (status / sent date / file link / customer notes) gated by a contract cap (over-cap visually flagged amber → billable), one-click **approval LOCK** (date + signer, freezes proof editing), **print runs** (qty + unit cost + paper + finish foil/emboss/letterpress/etc + envelope + printed/delivered + reprint-reason). Live totals: deliverables · proofs · qty · print cost. FunctionSheet.stationeryJson (BE `03497a7`, migration applied), card on detail view (FE `978743a`), flag `NEXT_PUBLIC_STATIONERY` default OFF + vendor-type-gated to Wedding Invitations and Stationery |
| 29 | 2026-05-24 | "keep going / continue" | Built **Henna / Mehndi Schedule** (§16.4, henna pillar) — per-subject row priced **per-hand × complexity tier** (the way PK henna artists actually charge, not flat), with live row total. Plus artist arrival + first-hand + last-finished times, per-subject appointment + actual start/finish (proof for "you came late" disputes), travel charge + out-of-city flag, family bulk discount, aftercare-kit toggle. Live totals strip: subjects · total hands · subtotal · grand. FunctionSheet.hennaJson (BE `1f38473`, migration applied), card on detail view (FE `e06c0bd`), flag `NEXT_PUBLIC_HENNA_SCHEDULE` default OFF + vendor-type-gated to Henna artist |
| 28 | 2026-05-24 | "keep going" | Built **Car Rental Fleet + Drivers** (§16.5, car-rental pillar) — per-vehicle assignment (driver/route/times/km/fuel/tolls/deposit/status/damage) + live convoy totals (km, fuel L, fuel+tolls Rs, deposits) + convoy notes. FunctionSheet.carRentalJson (BE `983315f`, migration applied), card on detail view (FE `49a5fbc`), flag `NEXT_PUBLIC_CAR_RENTAL` default OFF + vendor-type-gated to Car rental |
| 27 | 2026-05-24 | "keep going / continue" | Built **Decorator Setup + Inventory** (§16.6, decorator pillar) — setup/breakdown times + crew, per-item decor inventory with source/cost/condition (planned/setup/returned/damaged/lost) + live cost sum, substitution + damage-claim notes. FunctionSheet.decoratorSetupJson (BE `0f107b7`, migration applied), card on detail view (FE `53513b7`), flag `NEXT_PUBLIC_DECORATOR_SETUP` default OFF + vendor-type-gated to Decorator |
| 26 | 2026-05-24 | "keep going" | Built **Bridal-Wear Fitting Schedule** (§16.7, bridal pillar) — milestone timeline (Measurements→Cut→Stitched→Fittings→Final→Delivered) + per-outfit deposit/return/damage + alteration notes. FunctionSheet.bridalWearJson (BE `d5621dc`, migration applied), card on detail view (FE `27cc9f2`), flag `NEXT_PUBLIC_BRIDAL_FITTING` default OFF + vendor-type-gated to Bridal wearing |
