...
export const WeddingStationeryValidations = ({ currentStep, formData, currentErrors }: ValidationArgs) => {
    if (currentStep === 1) {
        ...
        if (!formData.downPaymentType) currentErrors.downPaymentType = 'Advance payment type is required';
        if (!formData.downPayment || Number(formData.downPayment) <= 0) {
            currentErrors.downPayment = 'Advance payment amount is required';
        ...
        if (!formData.subBusinessType || formData.subBusinessType.length === 0) {
            currentErrors.subBusinessType = 'Please select a shop type';
        }
        if (!formData.expertise || formData.expertise.length === 0) {
            currentErrors.expertise = 'Please select at least one product category you offer';
        }
    } else if (currentStep === 5) {
        if (!formData.instruction) currentErrors.instruction = 'Production turnaround time is required';
    } else if (currentStep === 6) {
        const imageCount = formData.imageFiles?.length ?? 0;
        if (imageCount === 0) {
            currentErrors.images = 'Please upload at least one portfolio image of your stationery work';
        } else if (imageCount > 20) {
            currentErrors.images = `Maximum 20 images allowed. Remove ${imageCount - 20} image(s).`;
        }
    } else if (currentStep === 7) {
        // Product listings are optional — only validate entries that exist
        const listings = formData.packages ?? [];
        listings.forEach((pkg, i) => {
            if (pkg.name?.trim() || pkg.price > 0) {
                if (!pkg.name?.trim()) {
                    currentErrors[`packages[${i}].name`] = 'Product name is required';
                }
