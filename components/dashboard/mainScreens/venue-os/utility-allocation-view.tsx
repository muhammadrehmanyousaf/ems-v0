      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2 rounded-md border p-3">
          <div className="flex flex-wrap items-end gap-2 text-sm">
            ...
            {/* WWL-563 / F11 — this was a raw "meter #" box asking for a primary
                key the vendor cannot know, while THIS SAME PANEL already has the
                meter list loaded a few lines above. Name them. */}
            <label className="flex flex-col gap-0.5 text-[11px] font-medium text-muted-foreground">
              Meter
              <select
                value={meterId}
                onChange={(e) => setMeterId(e.target.value)}
                className="w-48 rounded-md border border-input bg-background px-2 py-1 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <option value="">
                  {meters.length ? "Choose a meter…" : "Load meters first"}
                </option>
                {meters.map((m) => (
