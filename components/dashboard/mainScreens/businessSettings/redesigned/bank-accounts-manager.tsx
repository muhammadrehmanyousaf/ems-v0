...
export function BankAccountsManager() {
  ...
  const createMut = useMutation({
    mutationFn: () => BankDetailsAPI.create(form),
    onSuccess: () => { showSuccessToast("Bank account added"); setForm(EMPTY); setAdding(false); invalidate() },
    onError: (e: any) => toast.error(e?.response?.data?.message || e?.message || "Couldn't add account"),
  })
  ...
  const canSave = (form.bankName || "").trim() && (form.accountHolderName || "").trim() && (form.accountNumber || "").trim()
        ...
        {adding && (
          <div className="space-y-3 rounded-lg border border-primary/30 bg-primary/5 p-4">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5"><label className={labelCls}>Account number</label><input className={inputCls} value={form.accountNumber ?? ""} onChange={(e) => set("accountNumber", e.target.value)} placeholder="Account / 16-digit" /></div>
            ...
            <div className="flex gap-2">
              <Button size="sm" disabled={!canSave || createMut.isPending} onClick={() => createMut.mutate()}>{createMut.isPending ? <><Spinner size={14} className="mr-1.5" /> Saving…</> : <><Icon name="CheckCircle2" size={14} className="mr-1.5" /> Save account</>}</Button>
