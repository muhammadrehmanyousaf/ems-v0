...
export function PackagesManager({ businessId }: { businessId: number }) {
  ...
  const reset = () => { setForm({ name: "", price: "", description: "", features: "" }); setAdding(false) }
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { showSuccessToast("Package added"); reset(); invalidate() },
    onError: (e: any) => toast.error(e?.response?.data?.message || e?.message || "Couldn't add package"),
  })
        ...
        {adding && (
          <div className="space-y-3 rounded-lg border border-primary/30 bg-primary/5 p-4">
            ...
            <div className="flex gap-2">
              <Button size="sm" disabled={!canSave || createMut.isPending} onClick={() => createMut.mutate()}>{createMut.isPending ? <><Spinner size={14} className="mr-1.5" /> Saving…</> : <><Icon name="CheckCircle2" size={14} className="mr-1.5" /> Save package</>}</Button>
