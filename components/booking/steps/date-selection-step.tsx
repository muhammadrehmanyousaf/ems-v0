...
export default function DateSelectionStep({
  ...
  useEffect(() => {
    ...
    if (venue?.maxCapacity && initialCount > venue.maxCapacity) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">
          Pick a date, time slot, and expected guest count
        </p>
      </div>
        ...
        <div className="space-y-8">
          <div>
            ...
            >
              {timeSlots.map((slot) => {
                ...
                const isBooked =
                  selectedDayAvail?.bookedSlots?.includes(slot.value) ?? false;
                return (
                  <button
                    key={slot.value}
                    type="button"
                  ...
                  >
                    <div>
                      <p
                        className={`text-sm font-semibold ${isBooked ? "text-neutral-400" : isSelected ? "text-purple-700" : "text-neutral-700"}`}
                      >
                        {slot.label}
                      </p>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 flex items-baseline justify-center gap-1.5">
                <input
                  ...
                  className="w-16 text-right text-3xl font-bold text-neutral-900 bg-transparent border-0 outline-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none p-0"
                  placeholder="10"
                />
            ...
            {venue?.minCapacity &&
              ...
              formData.guestCount < venue.minCapacity && (
                <div className="mt-2 flex items-center gap-2 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
                  <AlertTriangle className="h-4 w-4 text-amber-500 shrink-0" />
                  <p className="text-xs text-amber-600">
