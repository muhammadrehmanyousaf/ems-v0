...
export function FeaturedBridalWear() {
  const [vendors, setVendors] = useState<Vendor[]>([])
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    const fetchFeaturedBridalWear = async () => {
      try {
        const vendorType = getVendorTypeFromPath('bridal-wear')
        const featuredBridalWear = await VendorAPI.getBusinessesByVendorType(vendorType)
        setVendors(featuredBridalWear.slice(0, 8)) // Limit to 8 featured
      } catch (error) {
        console.error('Error fetching featured bridal wear:', error)
      } finally {
        setIsLoading(false)
      }
    }

    fetchFeaturedBridalWear()
  }, [])
    ...
    <section className="py-6 sm:py-8 md:py-12 lg:py-16 bg-gray-50">
      <div className="w-full px-2 sm:px-4 md:px-6 lg:px-8 xl:px-12">
        <div className="flex justify-between items-end mb-12">
          ...
          <Link href="/vendors/bridal-wear" className="text-primary hover:underline hidden md:block">
        ...
        <div className="text-center mt-8 md:hidden">
          <Link href="/vendors/bridal-wear" className="text-primary hover:underline">
            View all bridal wear →
          </Link>
        </div>
      </div>
