  ...
  useEffect(() => {
    const fetchFeaturedVenues = async () => {
      try {
        ...
        const featuredVenues = await VendorAPI.getBusinessesByVendorType(vendorType)
        setVendors(featuredVenues.slice(0, 8)) // Limit to 8 featured
      } catch (error) {
      ...
      <div className="w-full px-2 sm:px-4 md:px-6 lg:px-8 xl:px-12">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-gray-600">Discover beautiful venues for your perfect wedding day</p>
          </div>
          <Link href="/venues" className="text-primary hover:underline hidden md:block">
            View all venues →
          </Link>
        </div>
