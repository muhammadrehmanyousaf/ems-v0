...
export function FeaturedWeddingStationery() {
  ...
  useEffect(() => {
    const fetchFeaturedWeddingStationery = async () => {
      try {
        const vendorType = getVendorTypeFromPath('wedding-stationery')
        const featuredWeddingStationery = await VendorAPI.getBusinessesByVendorType(vendorType)
        setVendors(featuredWeddingStationery.slice(0, 8)) // Limit to 8 featured
      } catch (error) {
        console.error('Error fetching featured wedding stationery:', error)
      } finally {
        setIsLoading(false)
      }
    }

    fetchFeaturedWeddingStationery()
  }, [])
    ...
    <section className="py-6 sm:py-8 md:py-12 lg:py-16 bg-gray-50">
      <div className="w-full px-2 sm:px-4 md:px-6 lg:px-8 xl:px-12">
        <div className="flex justify-between items-end mb-12">
          ...
          <Link href="/vendors/wedding-stationery" className="text-primary hover:underline hidden md:block">
