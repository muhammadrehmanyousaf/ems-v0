 ...
 *
 * Why split now even though we're under the 50k-URL limit:
 *   - Operationally cleaner — vendor adds don't invalidate the static
 *     sitemap shard, so static URLs keep their cache.
 *   - Lets us scale to 100k+ vendors later without restructuring.
 *   - Search Console reports per shard so we can spot indexing issues
 *     (e.g. "the vendors shard isn't indexing as fast as the cities shard").
 *
 * Reference:
 *   - https://nextjs.org/docs/app/api-reference/file-conventions/metadata/sitemap#generating-multiple-sitemaps
 *   - docs/seo/00-master-seo-playbook.md §1 item 28 (split when >50k or >50MB)
 *
 * Output URLs:
 *   /sitemap.xml             ← index of shards (auto-generated by Next.js)
 *   /sitemap/0.xml           ← `core`        — homepage, legal, hubs, tools (~50 URLs)
 *   /sitemap/1.xml           ← `programmatic` — city × vendor-type grid (~144 URLs)
 *   /sitemap/2.xml           ← `vendors`     — dynamic vendor leaf URLs (up to 50k each)
 *
 * If the vendors shard ever exceeds 50k, add additional shards by
 * paginating fetchDynamicVendors and returning more `id`s from
 * generateSitemaps below.
 */

const SHARDS = ["core", "programmatic", "vendors", "images"] as const
type Shard = (typeof SHARDS)[number]

export async function generateSitemaps() {
  return SHARDS.map((_, idx) => ({ id: idx }))
}
...
// ─── Default export — one shard per generateSitemaps id ─────────────────
...
export default async function sitemap({
  ...
  const shard: Shard = SHARDS[id] ?? "core"
  switch (shard) {
    case "core":
    ...
    case "images":
      return await buildImagesShard()
    default: {
      // Exhaustiveness guard — if a new shard is added to SHARDS without a
      // matching case, TypeScript flags it here.
      const _exhaustive: never = shard
      return _exhaustive
    }
  }
}
