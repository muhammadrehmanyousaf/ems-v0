...
const BUSINESS_SCOPED_PREFIXES = [
  ...
  "/api/v1/automation",
  /**
   * WWL-328 — `/api/v1/halal-certs` was missing, so the venue switcher did
   * nothing on the Halal register and certificates from all three venues merged
   * with no venue column — exactly the Inventory defect above. `listCerts`
   * reads `req.query.businessId` and always has; only the client never sent it.
   *
   * Drone NOC has the same shape: its list handler reads the parameter (and a
   * `bookingId` one beside it) and the client sent neither.
   */
  "/api/v1/halal-certs",
  "/api/v1/drone-noc",
  /**
   * WWL-421 — `listMyPromotions` ignored `req.query.businessId` and always
   * scoped to ALL the vendor's businesses, so the venue switcher did nothing on
   * this screen while the create dialog filed against a different venue. The
   * controller narrows on it now (never widening — it can only intersect with
   * the vendor's own ids), so the parameter is no longer noise.
   */
  "/api/v1/promotions",
];
